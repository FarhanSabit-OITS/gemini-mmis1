...
// Optimized Levenshtein Distance for robust fuzzy matching
...
export const Header = ({ 
  ...
  const registryMock = useMemo(() => [
    { type: 'VENDOR', name: 'Global Tech Solution', id: 'V-001', category: 'Registry Nodes', icon: Store, target: 'Vendors', status: 'Active', color: 'bg-indigo-500' },
    { type: 'VENDOR', name: 'Fresh Foods Co.', id: 'V-002', category: 'Registry Nodes', icon: Store, target: 'Vendors', status: 'Active', color: 'bg-indigo-500' },
    { type: 'VENDOR', name: 'Mukasa General Trade', id: 'V-003', category: 'Registry Nodes', icon: Store, target: 'Vendors', status: 'Pending', color: 'bg-amber-500' },
    { type: 'PRODUCT', name: 'Premium Basmati Rice', id: 'P-101', category: 'Catalog Items', icon: Package, target: 'Inventory Control', sub: 'Grain', color: 'bg-emerald-500' },
    { type: 'PRODUCT', name: 'Refined White Sugar', id: 'P-102', category: 'Catalog Items', icon: Package, target: 'Inventory Control', sub: 'Pantry', color: 'bg-emerald-500' },
    { type: 'PRODUCT', name: 'Solar Lantern X1', id: 'P-103', category: 'Catalog Items', icon: Package, target: 'Inventory Control', sub: 'Electronics', color: 'bg-emerald-500' },
    { type: 'ORDER', name: 'ORD-1001 (Maize Flour)', id: 'O-001', category: 'Order Manifests', icon: ShoppingCart, target: 'Orders', status: 'Pending', color: 'bg-blue-500' },
    { type: 'ORDER', name: 'ORD-1002 (Cooking Oil)', id: 'O-002', category: 'Order Manifests', icon: ShoppingCart, target: 'Orders', status: 'Shipped', color: 'bg-blue-500' },
    { type: 'ORDER', name: 'ORD-8821 (Industrial Salt)', id: 'O-003', category: 'Order Manifests', icon: ShoppingCart, target: 'Orders', status: 'Delivered', color: 'bg-blue-500' },
    { type: 'LOGISTICS', name: 'Weekly Bridge W21', id: 'L-001', category: 'Logistics Hub', icon: Truck, target: 'Supply Requisitions', status: 'En Route', color: 'bg-orange-500' },
    { type: 'MODULE', name: 'Revenue Analytics', id: 'MOD-REV', category: 'System Modules', icon: LayoutGrid, target: 'Revenue Module', color: 'bg-slate-700' },
    { type: 'MODULE', name: 'Security Console', id: 'MOD-SEC', category: 'System Modules', icon: FileText, target: 'Security Console', color: 'bg-red-600' },
    { type: 'USER', name: 'John Doe', id: 'U-1234', category: 'User Registry', icon: Users, target: 'Admin Roles', status: 'Verified', color: 'bg-purple-500' },
  ], []);
  ...
  const searchResults = useMemo(() => {
    ...
    const scoredResults = registryMock.map(item => {
      ...
      // 4. Fuzzy Levenshtein (Typo Tolerance)
        ...
        // Require relatively high similarity for fuzzy match to be relevant
                              ...
                              {group.items.map((item: any, j: number) => (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-white shadow-sm relative ${item.color || 'bg-slate-500'}`}>
                                      <item.icon size={16} />
                                      {item.score >= 1000 && (
                                        <div className="absolute -top-1 -right-1 bg-white text-emerald-600 rounded-full w-3.5 h-3.5 flex items-center justify-center ring-2 ring-white dark:ring-slate-900">
                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      <span className="text-[9px] opacity-40 font-mono tracking-tighter flex items-center gap-2">
                                        {item.id}
                                        {item.sub && <span className="bg-slate-100 dark:bg-slate-700 px-1 rounded text-[8px] text-slate-500 dark:text-slate-300">{item.sub}</span>}
                                      </span>
                                    </div>
                                  </div>
