...
export const VendorManagement = ({ user }: { user: UserProfile }) => {
  ...
  const kycInitialData = useMemo(() => {
    if (!viewingVendor) return null;
    return {
      firstName: viewingVendor.name.split(' ')[0] || '',
      lastName: viewingVendor.name.split(' ').slice(1).join(' ') || '',
      businessName: viewingVendor.name,
      idNumber: viewingVendor.id,
      city: viewingVendor.city,
      market: viewingVendor.market,
      businessType: viewingVendor.category,
  ...
  return (
    <div className="space-y-6 animate-fade-in relative pb-20">
          ...
          <div className="w-full max-w-xl h-full bg-white dark:bg-slate-900 shadow-2xl overflow-y-auto p-10 border-l border-slate-100 dark:border-slate-800 relative" role="dialog" aria-label="Vendor Details">
            ...
            <div className="space-y-8">
              ...
              <div className="pt-8 border-t border-slate-50 dark:border-slate-800 flex gap-4">
                 ...
                 {isAdmin && (
                   <Button variant="outline" onClick={openKYCView} className="flex-1 h-14 border-indigo-600 text-indigo-600 font-black uppercase text-xs rounded-2xl flex items-center gap-2">
                     <FileCheck size={18}/> View KYC
                   </Button>
                 )}
      ...
      {/* KYC Viewer Modal */}
      {showKYCView && viewingVendor && (
        <div className="fixed inset-0 bg-white dark:bg-slate-950 z-[300] overflow-y-auto animate-fade-in flex flex-col">
           <div className="max-w-5xl mx-auto w-full py-8 px-6">
              <div className="flex justify-between items-center mb-8 border-b border-slate-100 dark:border-slate-800 pb-4">
                 <div className="flex items-center gap-3">
                   <div className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center font-black shadow-lg">K</div>
                   <h3 className="text-xl font-black uppercase tracking-tight dark:text-white">Vendor Compliance Dossier</h3>
                 </div>
                 <button onClick={closeKYCView} className="p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors flex items-center gap-2">
                    <X size={24}/> <span className="font-black uppercase text-xs tracking-widest hidden sm:inline">Close Ledger</span>
                 </button>
              </div>
              <KYCModule 
                type="VENDOR" 
                userEmail={viewingVendor.email} 
                initialData={kycInitialData} 
                readOnly={true} 
                onComplete={closeKYCView} 
              />
           </div>
        </div>
      )}
